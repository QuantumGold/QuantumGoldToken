...
contract QuantumGoldTokenFactory {
    ...
    mapping(address => bool) public isQuantumGoldToken; //verify without having to do a bytecode check.
    bytes public quantumGoldByteCode;
    ...
    function verifyQuantumGoldToken(address _tokenContract) constant returns (bool) {
      ...
      if (fetchedTokenByteCode.length != quantumGoldByteCode.length) {
        return false; //clear mismatch
      }

      //starting iterating through it if lengths match
      for (uint i = 0; i < fetchedTokenByteCode.length; i ++) {
        if (fetchedTokenByteCode[i] != quantumGoldByteCode[i]) {
          return false;
        }
      }
